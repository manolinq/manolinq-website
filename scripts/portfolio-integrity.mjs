...
export async function verifyPortfolioOutput(){
 ...
 for(const file of ['index.html','work.html']){
  ...
  if(!html.includes('src="/scripts/portfolio-carousel.js"'))throw new Error(file+': missing carousel script reference');
  const script=await exactFile('site-dist','scripts/portfolio-carousel.js');
  if(!script.length||digest(script)!==digest(await exactFile('public','scripts/portfolio-carousel.js')))throw new Error('Carousel script missing or out of sync');
