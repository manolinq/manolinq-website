...
const contactLinks = [
  { href: 'mailto:support.manolinq@gmail.com',       icon: Mail,          label: 'support.manolinq@gmail.com', external: false },
  { href: 'https://wa.me/32456326720?text=Hey%2C%20I%27m%20interested%20in%20a%20website%20or%20AI%20system.%20Can%20you%20take%20a%20quick%20look%20at%20my%20business%3F', icon: MessageCircle, label: 'WhatsApp us directly', external: true  },
...
/**
 * Lead-capture contact form. POSTs to the Netlify Function which inserts into
 * the Supabase `leads` table and sends a Telegram notification. This component
 * is shared by the homepage contact section and the /contact page.
 */
export default function ContactForm({ animate = true }: { animate?: boolean }) {
  ...
  return (
    <div ref={ref as React.RefObject<HTMLDivElement>} className="grid lg:grid-cols-5 gap-14 lg:gap-20 items-start">

      {/* ── Left ── */}
      <div
        className={`lg:col-span-2 ${animate ? `transition-all duration-700 ${inView ? 'opacity-100 translate-x-0' : 'opacity-0 -translate-x-8'}` : ''}`}
      >
      ...
      {/* ── Right: Form ── */}
      <div
        className={`lg:col-span-3 ${animate ? `transition-all duration-700 delay-200 ${inView ? 'opacity-100 translate-x-0' : 'opacity-0 translate-x-8'}` : ''}`}
      >
        {submitted ? (
          <div
